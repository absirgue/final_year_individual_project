...
class IndustryOutlookEncode:
    ...
    def encode(self,weight_distribution):
        if len(weight_distribution) != 7:
            # print("INDUSTRY OUTLOOK ENCODING: Not enough weight provided")
            return self.data
        ...
        try:
            self.data['Industry Outlook Weighted Average'] = (self.data[industry_outlooks_col_names].mul(ordered_weights)).sum(axis=1)
